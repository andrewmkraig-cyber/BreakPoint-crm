...
// Ace-native mirror of EditableSkills. Same chip + add-on-Enter UX,
// writes to Neon directly via updateLocalCandidate instead of the
// RF API path. Skills back the search / Find Matches surfaces, so
// recruiters need to be able to add and remove them without
// re-uploading a resume.

export function LocalEditableSkills({
  candidateId,
  initial,
}: {
  candidateId: string;
  ...
  function persist(next: string[]) {
    ...
    setSkills(next);
    startSave(async () => {
      const result = await updateLocalCandidate({ id: candidateId, skills: next });
      if (!result.ok) {
        setSkills(snapshot);
