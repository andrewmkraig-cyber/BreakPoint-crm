...
import { createConsultingInvoice } from "@/app/invoices/consulting-invoice-actions";
...
// history table. The rows come down from the server page already
// serialized (amount as a number, dates as YYYY-MM-DD) so this file never
// touches prisma or Decimal.
...
export function ConsultingInvoicesSection({
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-court-border bg-court-surface-subtle/50">
                {["Invoice", "Company", "Amount", "Invoice Date", "Due Date"].map((h) => (
                  <th
                    key={h}
            ...
            <tbody>
              {visible.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-14 text-center text-[13px] text-court-fg-muted">
                    No consulting invoices yet.
                  </td>
                </tr>
              ) : (
                visible.map((inv) => (
                  <tr key={inv.id} className="border-b border-court-border last:border-b-0">
                    ...
                    <td className="px-6 py-3 align-top">
                      <div className="font-medium text-court-fg">
                        {CONSULTING_COMPANIES[inv.company].name}
                      </div>
      ...
      {open ? <GenerateConsultingInvoiceModal onClose={() => setOpen(false)} /> : null}
    </div>
  );
}

function GenerateConsultingInvoiceModal({ onClose }: { onClose: () => void }) {
  const router = useRouter();
  const today = todayIso();

  const [company, setCompany] = useState<"" | ConsultingCompanyKey>("");
  const [amountDigits, setAmountDigits] = useState("");
  const [invoiceDate, setInvoiceDate] = useState(today);
  const [dueDate, setDueDate] = useState(today);
  // Due date tracks the invoice date until the recruiter edits it directly.
  const [dueTouched, setDueTouched] = useState(false);
  const [periodStart, setPeriodStart] = useState(shiftIsoDate(today, -DEFAULT_SERVICE_PERIOD_DAYS));
  ...
  async function onSubmit() {
    ...
    try {
      ...
      setError("Something went wrong generating the invoice.");
      setSaving(false);
    }
  }
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Generate consulting invoice"
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            ...
            <Button
              ...
              className="w-auto"
            >
              {saving ? "Generating..." : "Generate and email"}
            </Button>
          </div>
        </div>
      </div>
