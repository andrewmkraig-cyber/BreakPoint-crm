...
//
// variant:
//   "lockup"   (default) — Serve Arc mark + Ace wordmark side-by-side.
//   "logo"     — Serve Arc mark only. Sized via the `size` prop.
//   "wordmark" — Ace + optional "by BreakPoint Talent" tag, no mark.
// The split lets the sidebar host the logo at the top (visual anchor)
// and the wordmark at the bottom (signature line) without two custom
// inline implementations drifting from this single source of truth.
type BrandMarkVariant = "lockup" | "logo" | "wordmark";
...
export function BrandMark({
  ...
  const showLogo = variant === "lockup" || variant === "logo";
  const showText = variant === "lockup" || variant === "wordmark";
  return (
