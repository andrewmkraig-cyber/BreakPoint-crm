...
// The Serve-Arc disc on its own. Extracted so the topbar lockup, the
// desktop sidebar, and the PWA drawer header all render the exact same
// mark from one source. The green arc + dot keep their fixed hex
// (#7BB85B) per the asset note above; only the soft glow tracks the
// court-brand token. Defaults to the 40px lockup size; pass `className`
// to resize/reposition for a given surface.
export function BrandDisc({ className }: { className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={cn(
        "ace-brand-disc relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[rgb(var(--court-brand)/0.35)] bg-[radial-gradient(circle_at_35%_30%,rgba(255,255,255,0.06),rgba(10,12,10,0.95)_45%,rgba(3,5,4,1)_100%)] shadow-[0_0_0_1px_rgb(var(--court-brand)/0.18),0_0_22px_rgb(var(--court-brand)/0.22),0_12px_28px_rgba(0,0,0,0.35),inset_0_1px_0_rgba(255,255,255,0.05),inset_0_-10px_20px_rgba(0,0,0,0.25)]",
        className,
      )}
    >
      <svg
