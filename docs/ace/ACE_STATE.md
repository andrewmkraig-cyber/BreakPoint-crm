...
- ~~**Temp `[web-push][diag]` log cleanup still pending.**~~ DONE Ace 89.0. All 10 `[web-push][diag]` console.logs removed from `web-push.ts` along with the orphaned `redactDigits` / `diagPayload` helpers (the light rebase). The functional Test button + delivery tally + `forceNotify` path were retained, so Bug 2 can still be monitored from Settings; only the console spam is gone. (Was tracked alongside the older temp-diag-log cleanup noted in `project_web_push_temp_diag`.)
