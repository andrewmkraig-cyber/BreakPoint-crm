...
**bookedAt on every billing event (`src/lib/billing-events.ts`).** A second date beside `scheduledAt`. A standard invoice books on the placement's start date (`placementBillingAnchor`), so a client's Net-10 / Net-30 terms can never move a deal across a quarter line. An installment books on ITS day of the schedule, start + `instNDaysAfterStart`, read off the "Installment N of M" note the Confirm Start flow writes (`installmentNumberFromNote`, now in the pure `placement-dates.ts` so the invoice re-dater and the event expander agree); the billing select carries `notes` for that. Only the payment-terms shift is ignored. `scheduledAt` keeps its due-date meaning ("when will the money land") for the Cash Forecast and Pipeline Value on the Scoreboard, which were deliberately left alone. Retained invoices have no start, so they book on their due date as before. The Billing Tower totals (`getBillingSummaryForRange`), both drill-downs (`/api/dashboard/billing-detail`, sorted and dated by `bookedAt`) and the jump-to-period bounds (`billing-period-options.ts`) all window on `bookedAt`.

**The first cut got installments wrong, and Andrew caught it from the number.** `09e8479e` booked every event on the start date, installments included. Q3 went from $123,125 to $124,375, not to $128,125: David came in at $5,000 but Ethan Larocca's second $3,750 installment (due 90 days after his June 1 start) fell back to Q2, and Latisha Ryder's 180-day second installment fell into her start quarter instead of Q2 2027, which also shrank the jump-to list. `9dac3b32` books installments on their own day.

**Read back against prod data after `9dac3b32`.** Q3 $128,125 (11 lines: David in at $5,000, Ethan's second installment stays on August 30), Q4 $6,000 (Latisha's first installment, October 15), Q2 2027 $4,000 (Latisha's second, April 3, 2027), so the tower's jump-to list reaches Q2 2027 again. The Goal Progress bar follows the same figure.
...
Four code commits, no schema change. Step 0 held at 3 / 10 / 85 before writing code and again before each commit.
...
**Consulting invoices send from AR, not andrew@ (`d29e2faf`).** Andrew's call this session. The From is the Accounts Receivable address from Billing settings (ar@breakpointtalent.com), the same default the placement invoice page already picks, sent as a verified "Send mail as" alias through Andrew's Gmail. `findVerifiedSendAs` in `src/lib/deals-alias.ts` generalises the deals@ check to any address and returns the alias display name for the From label; `canSendAsDeals` wraps it. If the account that sends has no verified AR alias, the mail goes out from that account's own address and the toast says so. Recipients (andrew@, austin@, owner's personal address on Cc) are unchanged.

**Split one fee between both companies (`7a809121`).** Andrew has been sending the two companies' invoices as 75 / 25 pairs by hand (7,500 / 2,500, 750 / 250, 450 / 150, 1,150 / 384). A Split toggle on the Generate modal replaces the Company select with an Arfie share percentage (default 75, `DEFAULT_CONSULTING_SPLIT_ARFIE_PCT`; Branzino shows the remainder), turns Amount into the total fee, and previews each share to the cent. `splitConsultingCents` rounds Arfie's share and hands Branzino the remainder so the two always add back to the total. Submit issues Arfie then Branzino through the same `createConsultingInvoice`, so each gets its own number, PDF and email from AR with the right owner on Cc; a failure on the second leaves the first issued and the error names it. The tiles on the section are plain per-company sums (Arfie $29,600.00, Branzino $3,784.00 on 2026-09-22), not a ratio of anything.
...
Six prompts plus a production hotfix. Every item pushed to main; `npm run build` exits 0 after each; all five `check:ui` gates green with no baseline raised. Step 0 ended at 3 / 10 / 85 (see ACE_RULES.md for why RfId moved 83 -> 85).

**Schema + permissions (`3b385df`).** New `Goal` and `GoalActualEntry` models, four enums (`GoalScope`, `GoalMetric`, `GoalPeriod`, `GoalStatus`), applied through the migration flow (`20260901141106_add_goals`, additive, zero DROP/TRUNCATE; prod data verified unchanged at 143 placements / 858 candidates / 11 invoices). `AVG_DEAL_SIZE` was added to `GoalMetric` in a second migration once the pacing work needed it. Money columns are `Decimal(14,2)` - one column that holds dollar targets with headroom and whole counts exactly. The three user FKs on `Goal` are scalar-only, the same choice `RetainedSearch` makes, to keep three more named back-relations off the NextAuth `User` model.

`src/lib/goals/permissions.ts` is the only reader of the two new nullable `User` columns, `goalLevel` and `managerId`. **These are NOT auth** - `UserRole` still decides what the app allows. A null `goalLevel` is the MOST JUNIOR rank, deliberately, so an unseeded user is never handed authority by accident. Every function takes an explicit `organizationId` and cross-checks it against the org stamped on each person; a cross-tenant argument THROWS rather than returning false, so it can never be mistaken for an ordinary denial. The manager walk is transitive with a visited set and a depth cap, so cyclic data cannot hang a request.

**The org chart is seeded and is a standing fact: Andrew `goalLevel` 0 (owner, the only approver), Austin `goalLevel` 1 reporting to Andrew (can REQUEST a company goal but not approve one).** `scripts/seed-goal-levels.ts` (dry-run default, `--apply`) writes it and is idempotent. This was found during prompt 5: both columns shipped nullable with no backfill, so until it ran **nobody could create a company goal at all** - `canRequestCompanyGoal` needs rank 0 or 1, so `createGoal` threw for Andrew too and the approval queue could never render.

`canSetGoalFor` was loosened in prompt 3 on Andrew's instruction to the rank rule: anyone at or below the actor's rank, anywhere in the org, plus the actor's own subtree, plus rank 0 reaching everyone. The subtree walk still earns its keep for a report whose `goalLevel` is null or seeded above their manager's. The only direction still denied is upward.

`scripts/seed-goals-2026.ts` seeded the live 2026 targets: 4 quarterly revenue at 125k, 4 quarterly signed-clients at 9, 1 annual revenue at 500k with `escalationPct` 15 and the quarterlies parented to it, and 1 lifetime milestone at 150k. Idempotent - a second `--apply` reports 10 already correct.

**Metric + pacing engine (`8de534f`, extended in `02f5681` / `383f1b4`).** One resolver per metric in `src/lib/goals/metrics.ts`, all org-scoped, all returning `number | null` where **null means "not measurable" and is never interchangeable with zero** - a null renders as a dash plus "not tracked yet", never as a miss.

Metric support, as measured against live data:
- **REVENUE** - supported, and returns THREE tiers from one call so they cannot be resolved separately and disagree: `earned` (`Placement.feeTotal` for placements PLACED in the window, retained excluded), `billed` (`Invoice.feeAmount` SENT+PAID by `sentAt`), `collected` (PAID only by `paidAt`). `earned` reads the SAME column `createInvoiceForPlacement` copies into `Invoice.feeAmount`, which is what stops the two drifting on one placement. `billed > earned` sets a `billedExceedsEarned` flag and is NEVER clamped - it means an invoice has no live placement behind it, and flooring it would hide a real data problem. Live 2026: earned $71,750 / billed $51,000 / collected $20,000.
- **PLACEMENTS, SIGNED_CLIENTS, INTERVIEWS, AVG_DEAL_SIZE** - supported. `AVG_DEAL_SIZE` returns null, never 0, when there were no placements.
- **SUBMITTALS** - supported, sourced from **`ActionLog`, not from `Placement`**. `stageMovedAt` only ever describes the CURRENT stage, so it cannot date a past transition; `ActionLog` is an append-only event log already recording `actionType: "submit"` with a real `createdAt` and `userId`, and the Scoreboard funnel reads the same rows, so the two agree by construction. De-duped by candidate + job: 100 raw rows -> 98 real submittals.
- **BD_CONTACTS_ENROLLED** - supported, summing `BDRun.enrolledCount` over runs completed in the window. `BDActivity`'s ENROLL rows are ONE ROW PER RUN with the count buried in `metadata.contacts` (39 rows describing 266 contacts), so counting them counts runs; the typed column and the JSON agree exactly at 266. `CampaignEvent` is an empty table.
- **BD_REPLIES** - **returns null, unsupported in v1.** The only table shaped to hold one is `BDActivity` kind=REPLY, fed by `/api/webhooks/apollo`, which has NEVER written a row (live `BDActivity` is 39 ENROLL and zero of everything else, consistent with the Apollo sequence Activate toggle staying off). A count would read 0 forever and look like failure rather than missing instrumentation.
- **MANUAL** - sums the goal's own `GoalActualEntry` rows.

Owner scoping uses only fields each model already has: Invoice and Placement via `client.ownerId`, Client via `ownerId`, ActionLog via `userId`, and **Interview via `createdById` - NOT `client.ownerId`, because only 11 of 78 live interviews carry a clientId and scoping through the client would drop 67 of them.** The BD tables record no user at all, so owner-scoped BD goals return null.

**MILESTONE REVENUE READS COLLECTED, NOT BILLED - a deliberate, scoped exception** (`revenueHeadline`). Every other period paces on billed, matching the 2026-05-26 Goal Pacing decision; a milestone is lifetime cash actually in the bank, which is what the seeded one says in its own notes. `AVG_DEAL_SIZE` deliberately does not take the exception. Live effect: the milestone reads $20,000 where the annual reads $51,000.

`src/lib/goals/pacing.ts` keeps THREE shapes apart on purpose. CUMULATIVE gets elapsed fraction, expectedToDate, paceIndex, projectedFinish, gap and daysRemaining, with day one counted as day 1 of N so the projection is defined from the first morning; a zero target and a not-yet-open period both yield a null pace index rather than a divide by zero, and "unknown" is never rendered as Behind. RATIO (`AVG_DEAL_SIZE`) gets NO expectedToDate and NO projection because an average converges rather than accumulating. MILESTONE gets percent complete, a trailing 90-day run rate, and a projected landing date that is stated in WORDS when the run rate is zero. Bands are 1.05 / 0.95 throughout.

**ET markers vs instants - the trap this arc kept hitting.** A goal's `periodStart`/`periodEnd` are UTC CALENDAR-DATE MARKERS, not instants: `2026-01-01T00:00Z` read as an ET wall clock is 7pm on Dec 31, which would pull the last evening of the prior quarter into Q1. `etWindow` reads each bound's UTC calendar date and anchors THAT date to ET midnight. The mirror-image bug bit twice more: `timeRange()` builds Month/Quarter/Year from server-LOCAL dates and Week from ET instants, so reading those back in the wrong frame put Q3 2026 at `2026-06-30T21:00Z` on a UTC+3 clock - inside the Q2 goal. **Rule: read a bound back in the frame it was written in, and compare marker to marker.** Vercel runs UTC so neither would have shown in prod. Verified identical under UTC, ET, Los Angeles, Tokyo and Moscow.

**The tab and its panels (`02f5681`, `383f1b4`).** Goals is a `?tab=goals` view with a sidebar row, wired exactly like Metrics and Placements - **there is no in-page tab strip on the dashboard; the section nav lives in the left sidebar.** Its period selector is goals-local (`?gperiod=`, the same separation Clubhouse has with `cbperiod`) because it needs a DAY grain the shared four-grain `TimeGrain` model does not have, and adding DAY to `TIME_GRAIN_ITEMS` would put a Day pill on Scoreboard, Placements, Clubhouse and the KPI dialog. It WRAPS `timeRange()` for the four shared grains so they cannot drift.

Six KPI tiles on the canonical `KpiTile` (which gained an optional `sub` caption - additive, no existing caller passes it). The revenue meter is the focal element: one bar with three NESTED fills (earned / billed / collected are the same money at three stages, so segments that add up would triple-count it), an expected-to-date marker rule, and every tier named in a legend AND printed as its own labelled figure - colour is never the only cue. Then the goal list (one row per goal whose window overlaps the selected one, milestones pinned last), the milestone tracker, the pace chart, and the client leaderboard.

**Charts here are pure CSS, no library and no SVG** - a rounded `bg-court-surface-subtle` track with an absolutely positioned `bg-court-brand-tint` fill grown by width or height %, the idiom the Scoreboard funnel and the Finances TrendCard already share. That idiom cannot express a two-series line, so the pace chart draws cumulative billed as a fill per bucket with the required pace as a thin rule across each column.

**Client leaderboard (`src/lib/goals/client-leaderboard.ts`).** Every filter is IMPORTED from `metrics.ts` - one definition each of billed, collected and countable placements, none restated - so summing the leaderboard reproduces the org-wide resolver figures exactly (verified on all four of collected, billed, earned and placements). `Client.feePct` is canonical with the typed `customFields` column as fallback through the existing shared parser; no new raw-blob read. `src/lib/client-identity.ts` is now the ONE definition of the `/clients/[id]` URL segment (`clientSlug` + `CLIENT_SLUG_SELECT`), which `src/lib/clients.ts` delegates to.

**Writes + approval queue (`b292587d`).** `src/app/dashboard/goal-actions.ts`: create, update, archive, approve, decline, addManualActual. `organizationId` and the acting user come from the SERVER SESSION on every call and never from client input; a client-supplied `goalId` or `ownerUserId` is verified to belong to the resolved org first. **Permission failures THROW rather than returning `{ ok, error }`** - a forbidden write means the caller reached a path they had no right to. Metric and period are FROZEN on an existing goal (changing either silently rewrites what every past reading meant); `updateGoal` accepts them only so a stale client is rejected loudly. Archive and decline set ARCHIVED and never hard-delete.

Verified against a production build by invoking the real server actions over `Next-Action`: Austin creates a company goal -> PENDING_APPROVAL; Andrew creates one -> ACTIVE self-approved; approve stamps `approvedByUserId` from the session; decline with a blank reason is refused and with a reason archives with the reason stored; **Austin approving throws (HTTP 500) and leaves the goal untouched**; the queue renders for Andrew and is hidden entirely from Austin, who still gets Add Goal. All test rows removed afterwards.

**Production hotfix (`a6defb4`) - PrismaClient bundled into the browser.** Sentry `ACE-CRM-1Y` on `/dashboard`. `goals-period-tabs.tsx` ("use client") -> `goals-period.ts` -> `metrics.ts` -> `@/lib/prisma`, whose module-scope `$extends` call throws on the browser shim's Proxy. Fixed by extracting `src/lib/goals/et-window.ts`; a second pre-existing instance in the Instantly settings view was fixed the same way. **New build gate `scripts/check-client-prisma.mjs`, wired into `check:ui`** - full rule in ACE_RULES.md. The lesson recorded there: `next build` exits 0, SSR renders, and the page returns 200 with this bug present - an HTTP 200 is not proof a page works.
...
Two items picked out of a code-level audit of Ace as a recruiting operating system. Both pushed to main; `npm run build` exits 0 after each. The audit surfaced 15 findings; the other 13 were explicitly left alone and are listed in ACE_ROADMAP.md under "Audit findings not yet actioned".

- **Prisma migrations baselined; `db push` retired (`546810b`).** Ace had no `prisma/migrations` folder. Every schema change since the start went to prod through `prisma db push`, which applies the schema and records nothing: no rollback, no history, no way to rebuild the database. **Checked before touching anything:** `migrate diff` from the live DB to `schema.prisma` returned an empty migration, so prod and the schema were already in exact sync, and no `_prisma_migrations` table existed. Neon point-in-time restore confirmed available (Andrew checked the console; retention 24h+). **The baseline:** `prisma/migrations/0_init/migration.sql`, generated with `migrate diff --from-empty`, verified against prod on every countable dimension - identical set of 78 table names, 285 indexes (208 CREATE INDEX + 77 PRIMARY KEY) vs 285 live, 139 FKs vs 139, 11 enums vs 11. Registered with `migrate resolve --applied`, so the SQL is recorded but was never executed against prod (`applied_steps_count = 0`). **No table data touched** - 141 placements / 857 candidates / 9 invoices / 26 clients / 50 jobs unchanged. **Pooler split:** the datasource gained `directUrl = env("DIRECT_URL")`. `DATABASE_URL` is the Neon POOLER endpoint and migrations cannot run through PgBouncer (transaction mode can't hold the advisory lock or the multi-statement DDL transaction); `DIRECT_URL` is the identical URL with `-pooler` removed from the host. Only `prisma migrate` reads it - verified `prisma generate` still exits 0 with it unset, so **the Vercel build needs no new env var**. `.env.local` and `.env.local.example` both updated. **The new workflow** is `npm run db:migrate -- <name>` (writes the SQL, touches nothing) then `npm run db:deploy` (applies it), with `npm run db:status` to see what's pending. Deliberately NOT `prisma migrate dev`: Ace has one database and it is production, and `migrate dev` spins up a shadow database and offers to RESET on drift. `scripts/new-migration.mjs` does the safe half (generate from a live-DB-vs-schema diff, no shadow DB, no reset path) and `migrate deploy` applies it. `scripts/prisma-with-env.mjs` exists because the Prisma CLI reads `.env`, not `.env.local`. `db:push` now exits with a pointer to the new flow rather than silently recreating drift. README gained a "Changing the database schema" section.

- **Days in Stage now reads a real stage-change timestamp (`50604eb`).** `/pipeline` computed Days in Stage from `Placement.updatedAt`, so editing a fee, a note or a billing contact reset the clock to zero - which silenced the 7-day amber pill, the 14-day red pill, and the Submitted follow-up prompt (gated on `> 2` days). The RF rows in the same table already used a real `stage_moved` value, so the two halves of one screen disagreed. **New `Placement.stageMovedAt`** (DateTime, `@default(now())`), shipped through the new migration flow as `20260830202113_add_placement_stage_moved_at`: added nullable, every existing row backfilled from `updatedAt` (141/141), then NOT NULL + default. Backfilling from `updatedAt` keeps every currently-displayed number identical on deploy; the column diverges and becomes correct from the next edit onward. **Maintained by a Postgres trigger, not application code.** There are 75 stage writes across 17 files; stamping each would be 75 chances to get it wrong now and a standing invitation to forget it on the next path added, and it could not distinguish a real transition from a no-op re-assert (the P2002 catch in `interview-actions.ts` re-asserts a stage the row may already hold) without an extra read per write. `placement_stamp_stage_moved_at()` fires `BEFORE UPDATE` and stamps only when `NEW.stage IS DISTINCT FROM OLD.stage`, which gets the no-op case exactly right and covers scripts and raw SQL too. An update that sets `stageMovedAt` WITHOUT touching `stage` passes straight through, so a data-fix script can still correct it by hand. **Readers repointed:** `pipeline/page.tsx` Days in Stage, plus the FOUR sites on `jobs/[id]/page.tsx` that fed a column literally named Stage Moved from `updatedAt` (the RF and Ace stage-overlay maps at lines 166/168, and both `extraRows` pushes) - same bug, second surface, found in the Step 0 grep. The amber pill, red pill and follow-up button all consume `daysInStage`, so they followed automatically with no edit. **`lastActionAt` deliberately left on `updatedAt`** - that column is "Last Action", and for an Ace row the last write genuinely is the last action; only the stage clock was wrong. **Verified against prod inside a rolled-back transaction:** a non-stage edit leaves the clock alone while `updatedAt` moves (proving the row was actually written), a no-op stage write leaves it alone, a real stage change bumps it, and a manual correction is respected. On a live submitted row sitting at 13 days, the old formula dropped it to 0d / grey / no-follow-up and the new one holds 13d / amber / follow-up shown. No production rows were modified.

**Permanent rule established:** never set `Placement.stageMovedAt` from application code, and never reintroduce `prisma db push`. Both are documented at the point of use (the `stageMovedAt` schema comment names the trigger and its migration; `db:push` fails loudly with the replacement flow).
...
- **Schema (`a7466396`).** New `RetainedSearch` (cuid pk, `organizationId`, required `clientId` + `jobId`, `totalAmount` Int, `paymentTerms` String default "Net 30", `guaranteeDays`, `useInstallments`, `status` via new `RetainedSearchStatus` enum OPEN/FILLED/CLOSED_UNFILLED, nullable `placementId`, `closedAt`, `closeReason`) indexed on `(organizationId, status)`. Nullable indexed `retainedSearchId` added to both `Placement` and `Invoice`. `Invoice.placementId` and `Invoice.candidateId` were ALREADY nullable, so no nullability change was needed for retained invoices, which carry neither. `totalAmount` is Int to match `Placement.feeTotal` rather than introducing a third money type alongside `Invoice.feeAmount`'s Decimal(10,2). FKs are scalar-only (no Prisma relations) so the two-way RetainedSearch/Placement link needs no disambiguating relation names. Applied via `db push`, no backfill.

- **Send Retained Invoice button + modal (`1a82a5f4`).** `/invoices` gained a second topbar action beside "+ New Invoice". The `Spec` type in `top-bar-page-title.tsx` already supported a second action via `extraAction` (as `/candidates` uses for Add Multiple), so no array refactor was needed. The chip routes through the shared `<Button variant="primary">`, whose classes already ARE the CTA treatment; the raw-button siblings in that file stay grandfathered. Modal fields in order: client (all clients, active and inactive), job filtered to that client, masked-currency retainer amount, terms picker, read-only due date recomputed live from today + terms, installments toggle with running total and mismatch warning, guarantee days with the "Clock starts on the candidate's start date, not today" hint. Reached from the topbar via a window event, the same bridge `CalendarNewEventButton` / `NewExpenseTopBarButton` use. Inline job creation was deliberately NOT built - `NewJobInput` needs ~15 fields and would have written half-populated Job rows - so the modal deep-links to `/jobs/new?clientId=`, a path that already existed. `createRetainedSearch` is org-scoped via `getCurrentOrg` (never from the caller) and validates client/job tenancy, positive whole-dollar amount and guarantee, a terms string the picker can actually produce, and that installments sum EXACTLY to the total.

- **Payment terms + installment storage + invoice generation (`953fa4cb`).** New `RetainedSearchInstallment` (sequence, amount, dueDate, nullable `invoiceId`), written in the SAME `prisma.$transaction` as the parent so a partial save is impossible. `PAYMENT_TERMS_DAY_OPTIONS` widened from `[0, 10, 14, 30]` to `[0, 10, 14, 15, 30, 45, 60]`; the picker and the server validator both derive from that one array, and the invoice editor's free-text terms field is untouched. On save, a DRAFT invoice is generated in the SAME `INV-####` sequence as every other invoice (shared `nextInvoiceNumber`, no separate series), with `retainedSearchId` set and `placementId`/`candidateId` null; single-payment writes one invoice for the full amount, installments write a live draft for #1 plus pre-staged `isFuture` drafts for the rest, stamp each invoice id onto its installment row, and create a 9:00 AM ET reminder 10 days before each later due date sliding to the prior Friday on a weekend - exactly what `confirmStart` already does for placement-side custom terms. Generation runs after the transaction commits and swallows its own failures, so a failed draft never costs the engagement record. The recruiter is routed to `/invoices/[id]` on success.

- **Placement-less PDF and email handling (`953fa4cb`).** Every spot assuming a candidate or placement now degrades: PDF line item reads `Retained search fee - [Job Title]` instead of `Placement Fee - [candidate]`; the CANDIDATE summary box becomes ENGAGEMENT / "Retained search"; START DATE prints "-" rather than echoing the issue date; PLACEMENT TYPE reads "Retained"; the line sub-text collapses to empty (no start date, no salary-derived fee basis); GUARANTEE reads `RetainedSearch.guaranteeDays`, falling through to the placement rules for every other invoice. The drafted email subject reads "[Role] retained search" and the opening line thanks the client for the engagement instead of congratulating them on a hire. ACCOUNT EXEC / COMPENSATION / FEE % already degraded to "-" on their own via the pre-existing optional chaining that the test-invoice path established. Recipients come from the client's own `Contact` rows split on the free-text title, because `Contact` carries no billing/hiring flag and there is no placement to read hand-entered lists from.

- **Retained Searches card + auto-link on hire + Close Search (`4514a7f8`).** New card directly above the placements ledger on the Placements tab, same big-panel chrome as breakdowns and map. Rooted at `RetainedSearch` and period-agnostic, so an OPEN search with no candidate still shows. Invoice Status reuses the ledger's `STATUS_LABEL`/`STATUS_PILL` over a status derived by the ledger's own `deriveBillingStatus` (both exported rather than duplicated); installment engagements read installment 1's invoice via `isFuture: false`. Guarantee counts down through `guaranteeDaysRemaining`, extracted out of `GuaranteePeriodTable` into `guarantee-period-utils` so both surfaces share one implementation; blank while OPEN since the clock starts on the candidate's start date. CLOSED_UNFILLED rows sort to the bottom at `opacity-60`, matching cancelled placements in the Hired pipeline tab. **Auto-link fires at the three FILL moments only** - `recordPlacement`, `recordLocalPlacement`, `confirmStart` - stamping `placement.retainedSearchId` and flipping the search to FILLED with its `placementId`. Offer-stage and every early-stage create path (applied / kept / submitted / interviewing / rejected, ~18 sites) are deliberately NOT wired, so a candidate merely touching a retained job never marks the engagement filled and a declined offer leaves no link. Multiple OPEN searches on one job link to the oldest and warn; the helper never throws. **`confirmStart` now SKIPS invoice creation entirely when the placement is retained** - two guards, one on the standard full-fee draft and one on the custom-terms installment block, both keyed on the resolved `retainedSearchId` - because the money is already invoiced on the search. Close Search sets CLOSED_UNFILLED with `closedAt` and a required short reason, is offered only on OPEN rows behind a confirmation dialog using the shared Button reject variant, and touches no invoice: money already sent or collected stays exactly as it is.

- **Revenue exclusion sweep (`8ed57f1b`).** The rule: a retained engagement's money is counted ONCE, from its invoice, and the placement that fills it contributes ZERO dollars anywhere. Two failure modes were in play - double-counting the fee, and dropping the retained invoice because it has no placement. **Making retained invoices countable:** `billing-events.ts` now loads retained invoices directly alongside placements (feeding Billing Tower Revenue / Outstanding / Goal Progress and both drill-downs), with ONE shared `invoiceToEvent` so placement and retained invoices get identical status gating and date windowing; `goal-pacing.tsx` and both `scoreboard-data.ts` queries (Pipeline Value, Cash Forecast) load them next to their placement query; `placements-dashboard.ts` selects `retainedSearchId`; `placement-drilldown` appends retained rows to `billed_revenue` and `cash_collected`, the latter having previously dropped them outright at its `.filter(Boolean)` on `placementId`. **Excluding the placement's fee:** `expandPlacementBillingEvents` returns `[]` when `retainedSearchId` is set, which zeroes the dollars across every consumer of the helper at once; non-dollar metrics (placement count, win rate, days to fill, offers) never route through it and are untouched. **A seventh double-count was found beyond the six audited paths:** `revenue-cards.tsx` selects uninvoiced placements with `invoices: { none: {} }`, which a retained placement satisfies because its invoice hangs off the search rather than the placement - so its `feeTotal` was being added on top of the retained invoice the query's other arm already counted. Fixed with `retainedSearchId: null`. **Avg Fee Size** now contributes `RetainedSearch.totalAmount` (the full engagement across all installments) for a retained placement, which would otherwise have counted as a $0 deal and dragged the average down; normal placements still read bare `feeTotal`, so the deliberate "typical deal size, not a sum of installments" decision stands for them. **Top Clients** folds in retained revenue from the same invoice events, since the filling placement contributes $0 by design and an OPEN search has no placement at all. Also fixed while in there: the hired-invoice lookup in `pipeline/page.tsx` had no `organizationId` on its where clause (rule 8).

**Verified for a $5,000 filled retained search:** $5,000 exactly once on Billing Tower Revenue and Outstanding, Goal Progress, both drill-downs, Goal Pacing, Scoreboard Pipeline Value, Cash Forecast, Avg Fee Size, Top Clients, and the `/invoices` KPI tiles; $0 contribution on the placements ledger (fee cell reads "-"), breakdowns, and map, where the retainer is represented by the Retained Searches card instead.
...
A BD-engine session that made discovery AND enrollment self-serve per saved search, and stopped the Active Campaigns screen from showing fake stats. All items pushed to main; `npm run build` exits 0 after each. Schema additions were additive only (the `BdSequence` table; `SavedSearch.theirstackSavedSearchId` / `runFrequencyDays` / `lastDiscoveredAt`), applied to prod via `db push`. In shipped order:

1. **Manufacturing vertical fully deleted (`04d6a6c`, applied to prod).** New idempotent, dry-run-by-default `scripts/delete-bd-vertical.ts` (`--name=`, `--org=`, `--apply`) retires an unused vertical plus everything cascading off it (SavedSearch + SavedSearchVersion, BdContactTargeting, BDRun + CampaignEvent/BDActivity, Campaign; ClientSignal is preserved via SetNull, verticalId nulled). Hard-blocks `--apply` if any LIVE run (QUEUED/RUNNING/AWAITING_APPROVAL/APPROVED) references the vertical. Ran `--apply` against prod to remove "Manufacturing".

2. **TheirStack search-by-ID, per saved search (`eb8e7a4`; titles-first companion `e2dd042`).** `SavedSearch.theirstackSavedSearchId` (nullable column) lets a saved search point at a real TheirStack saved search. When set, discovery FETCHES that saved search's stored filters (`GET /v0/saved_searches/{id}`) and REPLAYS them via `POST /v1/jobs/search` instead of the hardcoded `DISCOVERY_TITLES`. Blank = the default nationwide title search. Companion (`e2dd042`): when no TheirStack saved-search id is set, discovery now reads the selected vertical's titles (`BdContactTargeting.primaryTitles`) for `job_title_or` instead of the hardcoded Tax list, so the Verticals/Saved Search UI actually drives discovery (the titles-first item from ROADMAP ▸ NEXT (2)). The mandatory `posted_at_max_age_days` filter is still always sent (TheirStack Discovery Rule).

3. **Self-serve Apollo sequences (`6a40cc6`; seed `scripts/seed-bd-sequences.ts`).** New `BdSequence` table (org, name, apolloSequenceId, verticalId?, active; `@@unique([organizationId, apolloSequenceId])`). Settings ▸ BD now has add/edit/remove UI (`apollo-sequences-manager.tsx`; `createBdSequence`/`updateBdSequence`/`deleteBdSequence` in `settings/bd/actions.ts`), and the saved-search "Mapped Apollo Sequence" dropdown reads sequence NAMES from this table (`criteria.apolloSequenceId` stores the name string). `apollo-sequences.ts` is now the FALLBACK only when the table is empty. Seeded **Tax BD Sequence** (`6a06068f8142ee001d2b3dd2`) and **Great Neck BD** (`6a3057feaed6610020449ca9`).

4. **Per-saved-search schedule + pause (`25297f1`).** `SavedSearch.runFrequencyDays` (1=daily, 7=weekly, 15=every 15 days...) + `lastDiscoveredAt` (stamped each real cron run) + the existing `active` toggle. The 6 AM discovery cron runs a search only when it is `active` AND enough America/New_York days have passed since `lastDiscoveredAt`. "Run Discovery Now" ignores the schedule but still respects `active=false`. Defaults keep prior behavior (every active search runs daily).

5. **Date-window fix (`dda4bf4`).** Discovery's posted-at window now honors the saved search's own recency (derived from its last successful run) and the default max-age cap was raised to **30 days** (was effectively 7) so a first or infrequent run surfaces enough jobs. Dedup window stays 30 days (`DEDUP_WINDOW_DAYS`).

6. **Active Campaigns honesty (`d6724ef` + `3af1d36`).**
   - **Real Day-of counter (`d6724ef`).** Replaced the hardcoded "Day X of 7" with the run's real Apollo sequence step count, resolved most-specific-first (launch plan name -> linked Campaign apolloSequenceId -> saved search's mapped sequence name -> default sequence). Hidden entirely when no sequence resolves. Caveat noted in code: the X is days-since-enrolled and the denominator is the step count (Ace doesn't store per-step delays), so it's a cycle-progress estimate, the same shape the old "of 7" had.
   - **Fake stat tiles hidden (`3af1d36`).** The SENT/OPENED/REPLIED/BOUNCED/UNSUB tiles read from `CampaignEvent`, which nothing writes to yet, so they were always zero and misleading. When a row has no real `CampaignEvent` activity the tiles are hidden and replaced by a neutral "Live stats available in Apollo" note + an "Open in Apollo" deep link to that run's sequence (link resolves the real sequence id the same way enroll does, falling back to the Apollo sequences index so it's never dead). No numbers are invented; when real `CampaignEvent` rows exist the tiles render as before. Company names, the campaign list, and the Day-of counter are untouched. Display-only.

7. **Enroll sequence-routing fix (`8dd7a4c`).** `enrollCompaniesInApollo` resolved the Apollo sequence id ONLY from `APOLLO_SEQUENCE_ID` env or the first hardcoded `apollo-sequences.ts` entry, ignoring the run's saved search entirely - so a self-serve, table-only sequence ("Great Neck BD") enrolled into the wrong (Tax/default) sequence and showed ZERO in its own. New `resolveRunSequenceId` prefers the run's saved-search mapping: `criteria.apolloSequenceId` (name) -> `BdSequence` table (by name, then raw id) -> hardcoded `apollo-sequences.ts` -> `APOLLO_SEQUENCE_ID` env -> default. The org-wide cron (no saved search) keeps the original env->default path. **Guard:** if no valid 24-char Apollo sequence id resolves, the run now FAILS loudly (status `FAILED` + `errorMessage`) instead of calling `add_contact_ids` with an empty/garbage `emailer_campaign_id` and silently enrolling zero. **Logs** the resolved `emailer_campaign_id` per run for Vercel verification. Sequence resolution only - discovery, date window, mailbox rotation, and the contact-create call untouched.

**Data op - Great Neck dedup clear (`e70caeb`, applied to prod).** New idempotent, dry-run-by-default `scripts/clear-great-neck-today-dedup.ts`. The bd-discovery cron's dedup Set is built from the `discoveredPayload` of EVERY BDRun in the last 30 days regardless of status, so a company resurfaces only once it's gone from all recent payloads. The script scopes to the Great Neck saved search (via its mapped sequence, with a name fallback), and on `--apply` nulls `discoveredPayload` only on TODAY's finished (COMPLETE/FAILED) Great Neck runs - never Tax/nationwide runs, never live/pending runs. Ran `--apply` against prod: cleared 2 runs, freed 7 distinct companies (Harbor Accounting Group, Grassi, Great Hill Advisory, EY, JTS Associates CPAs P.C., JTS Associates CPAs, Whitman Advisory Job Community) so they can re-discover and re-enroll through the now-correct Great Neck routing.

**Supporting BD fixes this arc:** `add_contact_ids` mailbox key sent UNbracketed - a repeated `send_email_from_email_account_id` key, not `[]` - which was 422'ing rotation and adding nobody (`53eae51`); diag to surface a 2xx-but-zero-contacts `add_contact_ids` response + an all-unhealthy-mailbox fallback warning (`e1ca2c2`); dedup-clear script gained `--include-enrolled` (broad clear) + a recency window (`127c5b4`); the per-client TheirStack client-monitor sweep capped at 1 result per client (`4ac3cdf`); removed the inert "N companies up to N contacts" preview chip (`59bafba`).

**Also shipped (non-BD):** client name editable in the client Overview (`b8030d6`); mail search expanded to match by sent-recipient domains (`acbd7ff`); fixed a popped-out email window sticking during drag/resize (`5f6193c`); brightened the dark-mode date-picker icons (`ee2cbb1`).

**PAUSED - Active Campaigns REAL stats (CampaignEvent ingestion).** The original "show real Apollo activity" goal had three items; item 3 (real Day-of counter) shipped above, but items 1+2 (write `CampaignEvent` rows from the Apollo webhook + read real SENT/OPENED/REPLIED/BOUNCED/UNSUB) are PAUSED and need enroll-path changes. Findings (so a future session doesn't re-discover them): **no `Campaign` rows are ever created anywhere** (`prisma.campaign.create` exists nowhere), so `BDRun.campaigns` is always empty and the screen's `CampaignEvent groupBy` always returns zero - the read side is already correct, the gap is upstream. The Apollo webhook (`api/webhooks/apollo/route.ts`) writes only `BDActivity` and its assumed payload fields (`contact_email`, `sequence_id`) are UNVERIFIED guesses with no committed evidence of Apollo's real webhook schema; the payload carries no campaign/BDRun/contact id, so there is no usable campaign link. To wire it: create a `Campaign` row + per-contact `CampaignEvent` (incl. a `SENT`/`ENROLL` row) at enroll time, store enrolled-contact emails so the webhook can resolve a campaign by `contact_email`, and VERIFY Apollo's real webhook payload first. Until then the tiles stay hidden behind "Open in Apollo" (`3af1d36`).

**Known Apollo behavior (record - do not re-diagnose).** `add_contact_ids` SKIPS a contact already active in another sequence: Apollo returns it under `contacts_active_in_other_campaigns` and does NOT add it. A contact can be active in only ONE sequence at a time. Consequence for the Great Neck re-enroll: contacts that were wrongly added to the Tax sequence must be STOPPED/removed from Tax in Apollo before they will take in Great Neck - clearing the discovery dedup alone is not enough if the same person is still active in Tax.

**Cleanup flag.** As of this doc commit the canonical clone (`Projects/breakpoint-crm`) working tree is CLEAN. The previously-dirty Gmail mail-search files landed in `acbd7ff`; no `globals.css` change is pending here. If dirty Gmail/`globals.css` files are still visible, they are in the stale `Desktop/` clone (see `[[project_breakpoint_crm_canonical_clone]]`), not the canonical one - review/discard them there.
...
- **Agreement auto-fill on "Summarize Terms" (`b402b2d`, then fixed end-to-end `0233cd0`).** When summarizing an uploaded client agreement, Ace does a structured extraction of status/fee. **Fix (`0233cd0`):** the detected fields are now AUTO-WRITTEN to the client server-side, WRITE-IF-EMPTY, inside the summarize action itself - so the fee/signed/date actually populate end-to-end and survive a refresh. The original `b402b2d` only surfaced an ephemeral in-memory "Apply" box; a user who summarized and then refreshed/navigated away (as happens naturally) lost the detection and nothing was written (diagnosed against the live Mowat row: extraction returned {signed:true, feePercent:25} fine, but every client fee field was still null - the write step was the break, not extraction). No-clobber preserved (a field is filled only when currently empty, never overwriting a recruiter-set value); the manual confirm box now appears ONLY for genuine conflicts (detected value differs from an existing one) for an explicit overwrite. Verified end-to-end against the real Mowat row (null -> signed=true / 25% / Jun 12 2026).
- **Client grid card fee read fixed (`dfe3349`).** The `/clients` grid cards showed no "Fee X%" for Ace-native clients even after the fee was set. Root cause was a DUAL-FIELD READ: the cards derived the fee solely from the legacy RF `raw.custom_fields` ("Avg Fee %") blob, and `getClientsForOrg` didn't select the canonical `Client.feePct` column at all - and Ace-native rows have `raw === null`, so that read always returned null while the Overview (which reads `Client.feePct`) showed it. Fix: select `Client.feePct` and resolve canonical-first - `r.feePct ?? extractFeePctFromCustomFields(raw?.custom_fields)` (the shared helper as legacy fallback only for unbackfilled RF imports), so the card, the Overview, and `updateClientCompany` all read/write the ONE canonical field. Also added `revalidatePath("/clients")` to `summarizeAgreement` so the grid refreshes after auto-fill writes a fee. Verified against the live DB: Mowat (25%) and Tiret (20%) now render on their cards.
- **New build gate: legacy RF-blob read guard (`d3c05ba`).** Added `scripts/check-rf-blob-reads.mjs` + `scripts/rf-blob-baseline.json` (same pattern as the raw-button gate, wired into `npm run check:ui`) to stop the dual-field-read class of bug at the source. It blocks NEW reads of the legacy RF `raw` blob as a UI/logic data source - `.custom_fields` accesses and `.raw`/`.jobs ... as RF<Type>` casts - while grandfathering existing reads per file (fails only on a count increase or a new file; `--update` ratchets DOWN only). Constructed-literal mirrors (`{ ... } as RFJob`) and the allowlisted RF shape/normalize boundary (`src/lib/rf-payload-shapes.ts`) are excluded. Baseline at introduction: **16 reads / 5 files**, debt concentrated in `src/lib/candidates.ts` (5) and `src/lib/clients.ts` (4). Ratchet rule + `Client.feePct`-is-canonical note recorded in ACE_RULES.md.
