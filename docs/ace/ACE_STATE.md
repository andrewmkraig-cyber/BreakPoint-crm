...
- **Temp `[web-push][diag]` log cleanup still pending.** Diagnostic logging is live in prod (`web-push.ts`). Remove once push delivery + Bug 2 are settled; `web-push.ts` will want a light rebase when the logs come out. (Tracks alongside the older temp-diag-log cleanup noted in `project_web_push_temp_diag`.)
