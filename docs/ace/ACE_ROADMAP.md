...
- **Remove temp `[web-push][diag]` logging.** Live in prod (`web-push.ts`) for the Bug 2 investigation; pull once delivery + Bug 2 are settled. `web-push.ts` will want a light rebase when the logs come out. Tracks with the older temp-diag-log cleanup.
...
- **Remove temp `[web-push][diag]` logging** — live in prod (`web-push.ts`) for the Bug 2 investigation; pull once delivery + Bug 2 are settled (also tracked under Push notification follow-ups). `web-push.ts` will want a light rebase when the logs come out.
