...
1. **BD Engine block** — 12-19 hr total. Ships in Ace 40.0 after the Ace 40 design pass (Night Court light mode + Dashboard/Scoreboard/Invoicing redesign) is signed off. Claude Design pass first (1-2 hr) for BD tab + BD Settings + Sequence builder UI before any code.
   - ✅ **Phase 1 shipped in Ace 39.1 (2026-05-11)**: Prisma schema (Vertical, SavedSearch, SavedSearchVersion, SendingDomain, BDRun, Campaign, CampaignEvent, BDActivity, ClientSignal + 4 enums, all org-scoped), sidebar BD entry, `/bd` layout shell with 4-tab strip (Today's Launch / Client Signal / Active Campaigns / Activity), Today's Launch page with vertical segmented control + saved-search combobox + amber Launch CTA + confirmation modal + `POST /api/bd/runs` inserting BDRun status=QUEUED.
   - ✅ **Phase 2 shipped in Ace 39.2 (2026-05-12)**: Client Signal page (header + count badge + subtitle + TabStrip filter pills (All / New this week / Acted on / Dismissed) + stacked-row card with logo placeholder, primary contact, job title, location, posted-relative, View listing + disabled Reach out stub button + empty state). Active Campaigns page (one-row-per-BDRun with vertical pill, Day X of Y eyebrow, campaign name from SavedSearch, sub-line, metric strip (Sent / Opened%/Replied%/Bounced%/Unsub) driven by CampaignEvent groupBy, domain health 5-dot strip with current SendingDomain.status overlay, pause stub, chevron) + Campaign detail stub at `/bd/campaigns/[id]`. Activity page (TabStrip filter pills (All / Sends / Replies / Bounces / Domains) + Today/Yesterday/2-days-ago/Older grouping + 20px tone-colored glyph per kind + metadata-derived event text + right-aligned timestamp + 50-per-page cursor pagination via `?before=`). Plus the "~80 contacts → up to 80 contacts" copy fix on the Today's Launch preview chip — the daily cap is a ceiling, not a target.
   - ✅ **Phase 3 shipped in Ace 39.4 (2026-05-12)**: Full BD Settings page at `/settings/bd` with five CollapsibleSection cards: (1) Verticals & Saved Searches — accordion per Vertical with inline edit form (name, mapped sequence, contact cap, target-titles chip input, compound location rows, company size, boolean keywords, freshness dropdown); Save creates a new SavedSearchVersion every time so history is preserved (button reads "Save · creates v{n+1}"). (2) Apollo Integration — Connected/Not connected pill from APOLLO_API_KEY env presence, masked key display, Test connection button hitting new `/api/bd/apollo/test` (Apollo `/v1/users/me`), Rotate disabled with tooltip pointing at Vercel env, mapped sequences read-only table. (3) Sending Domains — table with Priority / Domain / Status pill / Reputation bar (hardcoded 85) / Inbox owner / Last cooldown, Add domain modal, inline Edit + Remove. (4) Daily Limits — Pause all toggle (gates `/bd/launch` Launch CTA via the new `BdOrgConfig.pauseAll` column), Global daily cap edit, per-vertical caps grid (`Vertical.dailyCap` column), 4 blackout-window pills (weekends, holidays, before 7am, after 5:30pm). (5) Reply Routing — webhook path display + last-reply timestamp + 3 toggle pills (forward to Apollo, auto-create candidate on positive reply, OOO filter). Schema bumps: `Vertical.dailyCap`, `SendingDomain.inboxOwner`, new `BdOrgConfig` model (single row per org with cap, pause-all, blackouts, reply routing toggles). Sticky in-page TOC at the top of the page using existing `SettingsTocLink` + section ids. `/bd/launch` now reads `pauseAll` and `globalDailyCap` from `BdOrgConfig` instead of hardcoded `false` / `80`.
   - Remaining sub-items in order: Apollo enrichment helper (standalone, reads Apollo /api/v1/people/search + /enrich), Indeed scan helper (standalone), Background job queue (Vercel Cron picks up QUEUED BDRuns at 6 AM ET), Apollo webhook for opens/replies/bounces (writes CampaignEvent + BDActivity rows the Phase 2 pages already render), Reach-out mail composer pre-fill from Client Signal, scheduled email send (Gmail API send-at), Sequence engine + Apollo sequence template wiring, Client Signal dismiss/acted-on flows, secure storage for `APOLLO_API_KEY` so the Rotate button can move it out of env, real Instantly reputation pull for the Sending Domains reputation bar.
