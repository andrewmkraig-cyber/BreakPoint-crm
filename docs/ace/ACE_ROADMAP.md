...
Ace 52.0 closed the bulk email Candidate Lists extension, the calendar create flow (grid slot + ComposeFAB New Event + New Reminder + TO/CC/BCC chip inputs with autocomplete + Teams OAuth reconnect), and ten of the seventeen visual redesign prompts (dashboard/clubhouse, pipeline, candidates search, candidate profile, jobs grid/detail, JD tab, clients, BD dashboard, BD settings, mail). Court Mode tokens enforced throughout per rule 12. Ace 53 opens on visual redesign Prompts 11-17 + the post-design fix list, then the remaining numbered sequence. Each numbered item ships start-to-finish before the next begins unless an explicit prereq is called out inline.

1. **Visual redesign pass — Prompts 11-17** — finish the remaining seven surfaces: phone, appearance (Settings ▸ Appearance), placements (dashboard tab), all settings (every Settings sub-page not already covered), scoreboard, applicants, finances (Clubhouse Finances tab). Same constraints as Prompts 1-10: Court Mode tokens only, screenshot-driven spec, do not move elements / change copy unless the spec explicitly calls for it.
