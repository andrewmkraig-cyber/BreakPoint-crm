...
- **Second icon fix — standalone-icon token sweep.** _Mobile-nav rainbow: ✅ DONE Ace 72.0_ — NAV_GROUPS + FOOTER_NAV (with the per-item `iconColor`) lifted into shared `src/components/nav-items.ts`; desktop `Sidebar` + `MobileNav` both consume it, so colors match and can't drift (rainbow kept, not flattened). _Standalone-icon token sweep: AUDITED Ace 72.0, no unambiguous fixes remained_ — across candidate/job/client/placement/pipeline (86 files, 392 icon usages) the action icons already inherit the right color from their wrapping Button and quiet row deletes already `hover:text-red-600`. Remaining are BUTTON-color judgment calls for Andrew, not icon-token bugs: (a) confirm checks use `emerald-600` vs the oracle's brand-green token; (b) a couple of standalone keep glyphs sit in `primary`-green buttons rather than the `keep` cyan variant; (c) `dismiss-placement-button` confirm is a neutral ghost with no green; (d) a few decorative accents use `text-brand-dark` (a token, not hex). Decide per-item before any further edits.
