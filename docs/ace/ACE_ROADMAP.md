...
Ace 55.0 redesigned the Applicants queue on the unified table card + chip spec (closes the applicants entry from Prompts 11-17). Ace 54.0 redesigned the Scoreboard (closes the scoreboard entry). Ace 53.0 unified every Settings sub-page on the same spec (closes the "all settings" + Settings ▸ Appearance entries). Ace 52.0 closed the bulk email Candidate Lists extension, the calendar create flow (grid slot + ComposeFAB New Event + New Reminder + TO/CC/BCC chip inputs with autocomplete + Teams OAuth reconnect), and ten of the seventeen visual redesign prompts (dashboard/clubhouse, pipeline, candidates search, candidate profile, jobs grid/detail, JD tab, clients, BD dashboard, BD settings, mail). Court Mode tokens enforced throughout per rule 12. Ace 56 opens on the remaining visual redesign prompts (phone, placements, finances) + the post-design fix list, then the remaining numbered sequence. Each numbered item ships start-to-finish before the next begins unless an explicit prereq is called out inline.

1. **Visual redesign pass — Prompts 11-17 (remaining)** — three surfaces still queued: phone, placements (dashboard tab), finances (Clubhouse Finances tab). Appearance + all-settings closed in Ace 53.0; scoreboard closed in Ace 54.0; applicants closed in Ace 55.0. Same constraints as Prompts 1-10: Court Mode tokens only, screenshot-driven spec, do not move elements / change copy unless the spec explicitly calls for it.
2. **Post-design fix list** — cleanup items surfaced during Prompts 1-10:
   - KPI card label alignment — partial fix landed in `9e8da16` (min-h-[32px] container); audit the full Clubhouse / Scoreboard / Invoices / Finances strips for the same baseline issue.
   - Scoreboard missing icons — Scoreboard panel headers lost their leading icons during the visual pass.
   - Section header icons — every dashboard `SectionHero` should carry a consistent leading icon; some surfaces dropped them.
   - Save Search button should be outlined, not filled — currently rendering as a filled brand-green button on `/candidates`; spec calls for outlined.
   - Candidates filter rail input consistency — pill / chip / select inputs in the rail use mixed heights and border styles after the visual pass; normalize to one input chrome.
3. **S3 Backup Cron** — pre-launch hardening follow-up to the Vercel Blob migration. Recurring cron mirrors the Blob bucket + critical Postgres tables (Candidate, Client, Job, Placement, Interview, CandidateResume metadata) to S3 so the team has off-platform restore points if Vercel or Neon ever goes sideways.
...
## Completed - Ace 52.0 Visual redesign pass (Prompts 1-10) + bulk email Lists + calendar create flow + UI polish (May 17, 2026)

Same-day follow-on after Ace 51.0 closed in the morning. Court Mode tokens enforced throughout per rule 12. Closes the bulk email Lists extension item carried into Ace 52 from the 51.0 known-issues list, the `+ New ▸ New Event / New Reminder` ComposeFAB item, and the Tighter applied-jobs strip item from Queued From Session.

- **Visual redesign pass — Prompts 1-10** — ten surfaces retuned to screenshot-driven specs: dashboard / Clubhouse (`cdf7ece`), pipeline (`9e08beb`), candidates search (`b09ab5e`), candidate profile (`da8992d`), jobs grid + detail (`ba38673`), Job Description tab (`2dfb72c`, `5629d89`), clients directory + detail (`a987d66`), BD dashboard (in-chain), BD settings panels (`5327215`), and mail (`76292b6`). Every spec hex mapped to a Court Mode token before write — hex-to-token mapping saved as Code memory so future redesign prompts can skip the mapping audit. Playfair Display references map to `font-serif` (project ships Bricolage Grotesque, not Playfair). Prompts 11-17 (phone, appearance, placements, all settings, scoreboard, applicants, finances) queued at the top of the active sequence.
- **Bulk email — Candidate Lists extension** (`0b9ba57`) — same `BulkEmailDialog` from the search-surface implementation wired into the Candidate Lists surface. Per-recipient merge field resolution + >25 confirm gate inherited. Closes the Ace 51 known-issue item.
- **Calendar create flow — end to end** — Calendar New Event modal with meeting type (Google Meet / Microsoft Teams) and provider routing (`2efad52`). Grid slot click pre-fills date + start/end time; ComposeFAB `+ New ▸ New Event` opens the same modal (`a0971e1`). CC chip field on the modal + tighter client picker mapping (`c47f88c`). Candidate/Client typeaheads on the event modal replaced with TO/CC/BCC chip inputs (`8c0729f`). Typeahead contact suggestions on those chip inputs reuse the mail composer's 3-source merge — Ace Candidates / Ace Contacts / Gmail Sent recipients (`f56b6be`). ComposeFAB gains New Reminder entry (`d710390`). Microsoft Teams OAuth reconnected after the Settings UI refactor.
- **Mini calendar prev/next month buttons wired** (`8f0e0ab`) — arrows previously no-op; now drive the visible month state.
- **Drop-up fix on typeahead** — TO/CC/BCC autocomplete dropdown flips upward when it would clip the viewport bottom; mail composer + event modal share the input component.
- **Applied-jobs strip tightened** (`d710390`) — closes the Tighter applied-jobs strip queued item.
- **Resume PDF viewer no longer clips** (`823ff9d`) — height cap on the candidate-profile embed PDF viewer removed.
- **DeleteClientButton fix** (`823ff9d`) — inlined to fix the missing-import error on `/clients/[id]`.
- **KPI tile label alignment** (`9e8da16`) — KPI tile label container gets `min-h-[32px]` for baseline alignment. Full audit across Clubhouse / Scoreboard / Invoices / Finances strips queued on the post-design fix list in the active sequence.
