...
1. **BD Engine block** — 12-19 hr total. Ships in Ace 40.0 after the Ace 40 design pass (Night Court light mode + Dashboard/Scoreboard/Invoicing redesign) is signed off. Claude Design pass first (1-2 hr) for BD tab + BD Settings + Sequence builder UI before any code.
   ...
   - Remaining sub-items in order: BD Settings page (verticals + saved searches + sending domains CRUD), Apollo enrichment helper (standalone), Indeed scan helper (standalone), Background job queue (Vercel Cron picks up QUEUED BDRuns at 6 AM ET), Apollo webhook for opens/replies/bounces (writes CampaignEvent + BDActivity rows that the Phase 2 pages already render), Reach-out mail composer pre-fill from Client Signal, scheduled email send (Gmail API send-at), Sequence engine + Apollo sequence template wiring, Client Signal dismiss/acted-on flows.
