...
Ace 56.0 redesigned the Finances tabs (Revenue & Profitability, Invoices, Expenses) on the unified card + chip spec (closes the finances entry from Prompts 11-17). Ace 55.0 redesigned the Applicants queue. Ace 54.0 redesigned the Scoreboard. Ace 53.0 unified every Settings sub-page. Ace 52.0 closed the bulk email Candidate Lists extension, the calendar create flow (grid slot + ComposeFAB New Event + New Reminder + TO/CC/BCC chip inputs with autocomplete + Teams OAuth reconnect), and ten of the seventeen visual redesign prompts (dashboard/clubhouse, pipeline, candidates search, candidate profile, jobs grid/detail, JD tab, clients, BD dashboard, BD settings, mail). Court Mode tokens enforced throughout per rule 12. Ace 57 opens on the remaining visual redesign prompts (phone, placements) + the post-design fix list, then the remaining numbered sequence. Each numbered item ships start-to-finish before the next begins unless an explicit prereq is called out inline.

1. **Visual redesign pass — Prompts 11-17 (remaining)** — two surfaces still queued: phone and placements (dashboard tab). Appearance + all-settings closed in Ace 53.0; scoreboard closed in Ace 54.0; applicants closed in Ace 55.0; finances closed in Ace 56.0. Same constraints as Prompts 1-10: Court Mode tokens only, screenshot-driven spec, do not move elements / change copy unless the spec explicitly calls for it.
