...
### The shared goal meter (Ace 99.3, updated Ace 100.1)
...
- **Headline meters lead with the achieved VALUE, not the percentage** (Ace 100.1). Revenue shows the dollars earned, the count meters show the count achieved, as the focal number in the large serif treatment; the percentage moves to the supporting line beneath, with target and window ("89% of 9 · Q3 2026"). `GoalMeter` takes a `focus` prop (`"percent"` | `"value"`, default `"percent"`); the headline cards pass `"value"`. This SUPERSEDES the earlier "percent complete is the largest element on every meter" rule.
- **The metric name is the largest text in the card header** (Ace 100.1). Revenue / Signed Clients / Placements render as a serif title above the focal number; the grain word (Yearly / Quarterly / Monthly ...) sits beside it, small and muted.
- **Days remaining renders once at page level when all three meters share a window** (Ace 100.1) - Month, Quarter and Year, where revenue and the count meters all resolve over the selected window, so the figure is identical on every card. At Day and Week the cards do NOT share a window (revenue follows the selector, the count meters floor to the quarter), so days remaining stays per-card and there is no page-level copy.
